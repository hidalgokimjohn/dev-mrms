...
if ($auth->is_loggedIn()) {
            ...
            <table class="table table-bordered " cellspacing="0" width="100%">
                ...
                if ($dqa->fk_finding !== 7) {
                    foreach ($display_findings as $display_finding) {
                        echo '<tr>';
                        if($display_finding['is_checked']==0){
                            echo '<td class="text-danger font-bold">' . $display_finding['findings'] . '</td>';
                        }else{
                            echo '<td class="text-navy font-bold">' . $display_finding['findings'] . '<br/><small><span class="fa fa-check-circle"></span> Complied</small></td>';
                        }
                        echo '<tr>';
                    }
                } else {
    ...
    <div class="form-group" id="">
        <button class="btn btn-primary btn-upload-file" <?php echo ($upload->can_upload <=1) ? '' : 'disabled'; ?>
